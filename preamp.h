...
private:
  ...
  static MultiStageVoice BuildMarkIICPlusVoice(const PreampProfile& profile) {
    ...
    voice.stages[0] = {
        ...
        0.22, 1.5, 0.0, -1.0, 0.55, 0.0, 0.15, 0.0, false};
    voice.stages[1] = {
        MakeStageSpec(profile.spec, 130.0, 1850.0, 4700.0, 150.0,
                      0.070, 1.85, 1.20, 0.98, 0.15),
        0.28, 2.0, 0.0, -1.5, 0.25, 0.0, 0.35, 0.01, false};
    voice.stages[2] = {
        MakeStageSpec(profile.spec, 180.0, 1650.0, 3900.0, 170.0,
                      0.100, 2.10, 1.32, 1.02, 0.20),
        0.32, 2.75, 0.0, -2.0, 0.10, 0.0, 0.50, 0.02, false};
    voice.stages[3] = {
        MakeStageSpec(profile.spec, 95.0, 2100.0, 3500.0, 88.0,
                      0.080, 1.86, 1.22, 0.97, 0.16),
        0.18, 0.75, 1.0, 0.0, 0.15, 0.0, 0.20, 0.0, true};
    voice.links[0] = {110.0, 7600.0, -1.1};
  ...
  static MultiStageVoice BuildFallbackVoice(const PreampProfile& profile) {
    MultiStageVoice voice;
    voice.stage_count = 3;
    voice.stages[0] = {profile.spec, 0.28, 1.0, 0.0, -1.0, 0.35, 0.0, 0.20, 0.0, false};
    voice.stages[1] = {profile.spec, 0.34, 2.0, 0.0, -2.0, 0.10, 0.0, 0.35, 0.01, false};
    voice.stages[2] = {profile.spec, 0.18, 0.0, 1.0, 0.0, 0.10, 0.0, 0.15, 0.0, true};
  ...
  TubeStageControls MakeStageControls(const MultiStageStageVoicing& voicing) const {
    ...
    controls.level_db = voicing.level_offset_db + controls_.level_db * voicing.level_scale;
    controls.bright_db =
